    ...
    def initiate_data_ingestion(self, data_path='notebook/data/paysim_fraud_data.csv'):
        ...
        try:
            ...
            train_set, temp_set = train_test_split(
                ...
                test_size=0.30, 
                random_state=42,
                stratify=df['isFraud']  # Maintain fraud ratio
            )
            ...
            logging.info(f"Train set: {len(train_set)} samples ({len(train_set)/len(df)*100:.1f}%) - Fraud rate: {train_set['isFraud'].mean()*100:.4f}%")
            logging.info(f"Val set: {len(val_set)} samples ({len(val_set)/len(df)*100:.1f}%) - Fraud rate: {val_set['isFraud'].mean()*100:.4f}%")
            logging.info(f"Test set: {len(test_set)} samples ({len(test_set)/len(df)*100:.1f}%) - Fraud rate: {test_set['isFraud'].mean()*100:.4f}%")
