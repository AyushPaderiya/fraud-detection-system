...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        try:
            ...
            df["errortoamountratio"] = np.where(df["amount"] > 0, total_error / df["amount"], 0)
            ...
            df["highrisknight"] = (df["ishighrisktype"] & df["isnighttransaction"]).astype(int)

            # High amount to customer (not merchant)
            df["highamountc2c"] = ((df["ishighamount"] == 1) & (df["ismerchantdest"] == 0)).astype(
                int
            )

            # Account drained to non-merchant
            df["drainednonmerchant"] = (
                (df["isorigindrained"] == 1) & (df["ismerchantdest"] == 0)
            ).astype(int)
...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, val_path, test_path):
        ...
        try:
            ...
            logging.info(
                f"After feature engineering - Train: {train_df.shape}, Val: {val_df.shape}, Test: {test_df.shape}"
            )
            ...
            logging.info(
                f"Transformed shapes - Train: {X_train_arr.shape}, Val: {X_val_arr.shape}, Test: {X_test_arr.shape}"
            )
